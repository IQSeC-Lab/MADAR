...
def IFS_Samples_BOSS_V1(model, v, Y, v_choose, get_anomalous=True, contamination=0.1, madarEL2N=False):
    data_X = v
    ...
    if get_anomalous:
        ...
        if len(anomalous_samples_pool) > v_choose_split:
            if madarEL2N:
                # *** Use boss_selection instead ***
                selected_indices = boss_selection(model, np.array(anomalous_samples_pool), 
                                                 np.ones(len(anomalous_samples_pool)) * Y, 
                                                 budget_per_class=v_choose_split) 
                anomalous_samples = np.array(anomalous_samples_pool)[selected_indices]
        ...
        if len(anomalous_samples) == v_choose_split:
            if madarEL2N:
                # *** Use boss_selection instead ***
                selected_indices = boss_selection(model, np.array(similar_samples_pool), 
                                                 np.ones(len(similar_samples_pool)) * Y, 
                                                 budget_per_class=v_choose_split)
                similar_samples = np.array(similar_samples_pool)[selected_indices]
...
def IFS_Samples_Prior_BOSS(model, v, Y, v_choose, get_anomalous=True, contamination=0.1, madarEL2N=False):
    data_X = v
    clf = IsolationForest(max_samples=len(data_X), contamination=contamination)
    clf.fit(data_X)
    y_pred = clf.predict(data_X)
    ...
    if get_anomalous:
        ...
        if len(anomalous_samples_pool) > v_choose_split:
            if madarEL2N:
                el2n_subset, selected_indices, selected_labels, el2n_scores = el2n_subset_selection(
                    model, anomalous_samples_pool, np.ones(len(anomalous_samples_pool)) * Y, v_choose_split, num_classes = 100, batch_size=256)
        ...
        if len(anomalous_samples) == v_choose_split:
            #print(similar_samples_pool.shape)
            if madarEL2N:
                #print(f'number of samples in the pool {len(similar_samples_pool)}')
                #print(similar_samples_pool)
                el2n_subset, selected_indices, selected_labels, el2n_scores = el2n_subset_selection(
                    model, np.array(similar_samples_pool), np.ones(len(similar_samples_pool)) * Y,\
                    v_choose_split, num_classes = 100, batch_size=256)
            ...
            if len(similar_samples_pool) > v_choose_split:
                ...
                if madarEL2N:
                    el2n_subset, selected_indices, selected_labels, el2n_scores = el2n_subset_selection(
                    model, similar_samples_pool, np.ones(len(similar_samples_pool)) * Y, v_choose_split, num_classes = 100, batch_size=256)
                    similar_samples = el2n_subset
                else: 
                    similar_samples = random.sample(similar_samples_pool, v_choose_split)
            else:
        ...
        if len(similar_samples_pool) > v_choose:
            if madarEL2N:
                    el2n_subset, selected_indices, selected_labels, el2n_scores = el2n_subset_selection(
                    model, similar_samples_pool, np.ones(len(similar_samples_pool)) * Y, v_choose, num_classes = 100, batch_size=256)
                    similar_samples = el2n_subset
            else:
                similar_samples = random.sample(similar_samples_pool, v_choose)
        else:
...
def IFS_Prior_BOSS(model, prevX, prevY, memory_budget, min_samples=1, fs_option='ratio'):
    uniqueY = np.unique(prevY)
    ...
    for indx, Y in enumerate(uniqueY):
        ...
        if len(X_) <= v_choose:
            ...
            #print(f'len(X_) {len(X_)} > v_choose {v_choose}')
            X_ = IFS_Samples(model, X_, Y, v_choose, get_anomalous=True, contamination=0.1, madarEL2N=True)
            
            # print(f'X_ {X_.shape} Y_{Y_.shape}')
            # el2n_subset, selected_indices, selected_labels, el2n_scores = el2n_subset_selection(
            #       model, X_, Y_, v_choose, num_classes = 100, batch_size=256)
            # X_ = el2n_subset
            # print(f'X_ {len(X_)}')
...
#   if method == "gradient":
#     data.requires_grad = True  # Enable gradient calculation for the input data
#     outputs = model(data)
#     loss = torch.nn.functional.cross_entropy(outputs, labels)  # Example loss function
#     gradients = torch.autograd.grad(loss, data)[0]  # Get gradients w.r.t. input
#     importance_scores = torch.abs(gradients).mean(dim=1).detach().cpu().numpy()

#   elif method == "perturbation":
#     # Make sure your model has a predict method, or replace this with
#     # the appropriate way to get predictions from your model
#     baseline_predictions = model.predict(data)  
#     importance_scores = np.zeros(data.shape[0])
#     for i in range(data.shape[1]):  
#       perturbed_data = data.clone().detach()  # Create a copy for perturbation
#       perturbed_data[:, i] += 0.01  # Add a small perturbation
#       perturbed_predictions = model.predict(perturbed_data) 
#       importance_scores += np.abs(perturbed_predictions - baseline_predictions).mean(axis=1)
#   else:
...
#     class_indices = torch.where(labels == class_id)[0]  # Get indices for the current class
#     class_data = data[class_indices]
#     class_labels = labels[class_indices]

#     # Calculate importance scores for the current class
#     importance_scores = calculate_importance_scores(model, class_data, class_labels, method="gradient") 

#     # Sort by importance and select the top samples within the budget
#     _, sorted_indices = torch.sort(torch.tensor(importance_scores), descending=True)
...
# def IFS_Samples(model, v, Y, v_choose, get_anomalous=True, contamination=0.1, madarEL2N=False):
#   data_X = v.cpu().numpy()  # Convert to NumPy for IsolationForest
#   clf = IsolationForest(max_samples=len(data_X), contamination=contamination)
#   clf.fit(data_X)
#   y_pred = clf.predict(data_X)
#   anomalous_idx = np.where(y_pred == -1.0)
...
#           if madarEL2N:
#               # *** Use boss_selection instead ***
#               selected_indices = boss_selection(model, v, Y, budget_per_class=v_choose_split)
#               anomalous_samples = v[selected_indices].cpu().numpy()  # Select from original tensor and convert to NumPy
#           else:
#               anomalous_samples = random.sample(anomalous_samples_pool, v_choose_split)
#       else:
#           anomalous_samples = anomalous_samples_pool
...
#           if madarEL2N:
#               # *** Use boss_selection instead ***
#               selected_indices = boss_selection(model, torch.tensor(similar_samples_pool), 
#                                                Y, 
#                                                budget_per_class=v_choose_split)
#               similar_samples = torch.tensor(similar_samples_pool)[selected_indices].cpu().numpy()
#           else:
#               similar_samples = random.sample(similar_samples_pool, v_choose_split)
...
# def IFS(model, prevX, prevY, memory_budget, min_samples=1, fs_option='ratio'):
#   uniqueY = torch.unique(prevY) # Assuming prevY is a PyTorch tensor
  
#   countDict = {}
#   numSample = 0
#   for tmpY in uniqueY:
#       tmp_YSample = len(torch.where(tmpY == prevY)[0])
#       countDict[tmpY.item()] = tmp_YSample  # Use .item() to get the value from the tensor
...
#   for indx, Y in enumerate(uniqueY):
#       Y_ind = torch.where(Y == prevY)[0]
#       X_ = prevX[Y_ind]
#       Y_ = prevY[Y_ind]
      
#       if fs_option == 'ratio':
#           v_choose = int(np.ceil((countDict[Y.item()] / numSample) * memory_budget))
...
#           for sample in X_:
#               X_replay.append(sample.tolist()) # Convert NumPy array to list
#               Y_replay.append(Y.tolist()) # Convert PyTorch tensor to list

#   X_replay, Y_replay = torch.tensor(X_replay), torch.tensor(Y_replay) # Convert lists back to PyTorch tensors
...
def calculate_importance_scores_V1(model, data, labels, method="gradient"):
    """
    Calculates importance scores for 1D data using an MLP model.
    ...
    if method == "gradient":
        ...
        baseline_predictions = torch.argmax(model(data), dim=1)  # Fixed predict method
        importance_scores = np.zeros(data.shape[0])
        data = data.unsqueeze(1) if data.ndim == 1 else data  # Ensure 2D shape

        for i in range(data.shape[1]):
            perturbed_data = data.clone().detach()
            perturbed_data[:, i] += 0.01
            perturbed_predictions = torch.argmax(model(perturbed_data), dim=1)
...
def IFS_Samples(model, v, Y, v_choose, get_anomalous=True, contamination=0.1, madarEL2N=False):
    ...
    anomalous_samples_pool = data_X[anomalous_idx].tolist()
    similar_samples_pool = data_X[similar_idx].tolist()

    v_choose_split = int(np.ceil(v_choose / 2))
    #anomalous_samples = random.sample(anomalous_samples_pool, min(v_choose_split, len(anomalous_samples_pool)))
    
    #print(f'v {v.shape} Y {Y.shape} np.unique(Y) {np.unique(Y)} type(Y)')
    
    if madarEL2N:
        #print(f'anomalous_samples_pool {len(anomalous_samples_pool)} ')
        anomalous_budget = min(v_choose_split, len(anomalous_samples_pool))
        
        selected_indices = boss_selection(
                            model,
                            anomalous_samples_pool,
                            torch.tensor(np.ones(len(anomalous_samples_pool)) * np.unique(Y)[0]),
                            budget_per_class=anomalous_budget
                        )

        
        # anomalous_samples = v[selected_indices].cpu().numpy()
        
        # remaining_samples = v_choose - len(anomalous_samples)
        
        # selected_similar_indices = boss_selection(
        #                             model,
        #                             similar_samples_pool,
        #                             torch.tensor(np.ones(len(similar_samples_pool)) * np.unique(Y)[0]),
        #                             budget_per_class=min(remaining_samples, len(similar_samples_pool))
        #                         )
        
        # similar_samples = v[selected_similar_indices].cpu().numpy()
        ...
        similar_samples = similar_samples_pool[selected_similar_indices] #.cpu().numpy()        
    
        #similar_samples = random.sample(similar_samples_pool, min(remaining_samples, len(similar_samples_pool)))
    
    # replay_samples = np.concatenate((anomalous_samples, similar_samples)) if anomalous_samples and similar_samples else anomalous_samples or similar_samples
    if isinstance(anomalous_samples, list):
        anomalous_samples = np.array(anomalous_samples)
    if isinstance(similar_samples, list):
        similar_samples = np.array(similar_samples)
    
    replay_samples = np.concatenate(
                        (anomalous_samples, similar_samples)
                    ) if anomalous_samples.size > 0 and similar_samples.size > 0 else (
                        anomalous_samples if anomalous_samples.size > 0 else similar_samples
                    )
...
def IFS(model, prevX, prevY, memory_budget, min_samples=1, fs_option='ratio'):
    prevX = torch.tensor(prevX) if isinstance(prevX, np.ndarray) else prevX
    prevY = torch.tensor(prevY) if isinstance(prevY, np.ndarray) else prevY

    
    uniqueY = torch.unique(prevY)
    countDict = {tmpY.item(): len(torch.where(tmpY == prevY)[0]) for tmpY in uniqueY}
    numSample = sum(countDict.values())
    ...
    for Y in uniqueY:
        ...
        X_ = prevX[Y_ind]
        Y_ = prevY[Y_ind]
        #print(f'memory_budget {memory_budget} X_ {X_.shape} Y_ {Y_.shape}')
        v_choose = int(np.ceil((countDict[Y.item()] / numSample) * memory_budget)) if fs_option == 'ratio' else int(np.ceil(memory_budget / len(uniqueY)))

        v_choose = max(v_choose, min_samples) if fs_option == 'mix' else v_choose

        if len(X_) <= v_choose:
            X_replay.extend(X_)
            Y_replay.extend(Y_.tolist())
        else:
            X_ = IFS_Samples(model, X_, Y_, v_choose, get_anomalous=True, contamination=0.1, madarEL2N=True)
            X_replay.extend(X_.tolist())
            Y_replay.extend([Y.item()] * len(X_))

    if len(X_replay) > 0:
        X_replay, Y_replay = torch.tensor(X_replay), torch.tensor(Y_replay)
    else:
        X_replay, Y_replay = torch.empty(0), torch.empty(0)
...
def getReplaySamples(model, PreviousTasksData,
    ...
    if args.replay_config == 'frs':
        ...
        all_replay_X, all_replay_Y = IFS(model, all_replay_X, all_replay_Y,\
...
#         print(f'\n\naccs {accs}\n precisions {precisions}\n recalls {recalls}\n f1scores {f1scores}', flush=True)
        ...
        #     result_string = '{}\t{:.5f}\t{}\t{:.5f}\t\n'.format(task, np.mean(accs), num_training_samples, task_training_time)
        ...
        results_f = open(args.r_dir, 'a') 
        result_string = '{}\t{:.5f}\t{}\n'.format(task, np.mean(accs), num_training_samples)
        results_f.write(result_string)
        results_f.flush()
        results_f.close() 
